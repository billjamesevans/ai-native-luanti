...
local player_agent_loop = core.ai_agent_plugin.get_player_state("Wills").loop
assert(player_agent_loop ~= nil)
assert(player_agent_loop.status == "pending_approval")
assert(player_agent_loop.phase == "awaiting_player_approval")
assert(player_agent_loop.active_goal == "build marker")
assert(player_agent_loop.active_surface == "builder")
assert(player_agent_loop.next_action == "wait_for_player_approval")
assert(player_agent_loop.last_trace_id == build_reply.trace_id)
assert(player_agent_loop.last_result_status == "pending_approval")
assert(player_agent_loop.iteration >= 1)
assert(player_agent_loop.last_observation ~= nil)
assert(player_agent_loop.last_observation.action == "build")
assert(player_agent_loop.last_observation.surface_id == "builder")
assert(player_agent_loop.last_observation.anchor_node_name == "air")
core.step_ai_tasks()
assert(get_test_node(build_pos).name == "air")
...
local player_agent_loop_after_approve = core.ai_agent_plugin.get_player_state("Wills").loop
assert(player_agent_loop_after_approve.status == "queued")
assert(player_agent_loop_after_approve.phase == "acting")
assert(player_agent_loop_after_approve.next_action == "step_ai_task_queue")
assert(player_agent_loop_after_approve.last_task_id == approved_build.task_id)
assert(player_agent_loop_after_approve.last_result_status == "queued")
...
local fire_only_pos = test_pos(4229)
set_test_node(fire_only_pos, { name = "air" })
local fire_only_build = core.ai_agent_plugin.handle_command(
...
assert(fire_only_build.ok == true)
assert(fire_only_build.action == "build")
assert(fire_only_build.status == "pending_approval")
assert(fire_only_build.build_kind == "fire")
assert(fire_only_build.build_material_name == "fire")
assert(fire_only_build.build_material_node == "ai_runtime_test:fire")
assert(fire_only_build.planned_node_writes == 1)
assert(get_test_node(fire_only_pos).name == "air")
local fire_only_trace = core.ai_agent_plugin.get_request_traces({ limit = 1 })[1]
assert(fire_only_trace.public_prompt == "build me a fire and only a fire")
assert(fire_only_trace.route == "deterministic_build_parser")
assert(fire_only_trace.context.build_kind == "fire")
assert(fire_only_trace.context.build_material_name == "fire")
assert(fire_only_trace.response.planned_node_writes == 1)
local discarded_fire_only = core.ai_agent_plugin.handle_command("Wills", "discard plan", {})
assert(discarded_fire_only.ok == true)
assert(discarded_fire_only.action == "discard_approval")
assert(get_test_node(fire_only_pos).name == "air")
...
assert(type(async_planner_calls[1].context.player_agent_loop) == "string")
local planner_player_agent_loop =
	core.parse_json(async_planner_calls[1].context.player_agent_loop)
assert(planner_player_agent_loop.status == "running")
assert(planner_player_agent_loop.phase == "observing")
assert(planner_player_agent_loop.active_goal == "build a small shelter")
assert(planner_player_agent_loop.active_surface == "builder")
assert(planner_player_agent_loop.next_action == "reason_with_tools")
assert(planner_player_agent_loop.last_observation ~= nil)
assert(planner_player_agent_loop.last_observation.action == "build")
...
local pending_options = core.ai_agent_plugin.handle_command("Planner", "options", {})
assert(pending_options.ok == true)
assert(pending_options.action == "build_options")
assert(pending_options.status == "success")
assert(pending_options.pending_action == "build")
assert(pending_options.selected_candidate_id == "platform")
assert(pending_options.candidate_count >= 3)
assert(#pending_options.candidate_options >= 3)
assert(pending_options.candidate_options[1].option_id ~= nil)
local first_option_plan = pending_options.candidate_options[1].openrealm_plan
assert(first_option_plan ~= nil)
assert(first_option_plan.schema_version == "openrealm.plan.v1")
assert(first_option_plan.product == "OpenRealm")
assert(first_option_plan.assistant == "Nova")
assert(first_option_plan.plan_kind == "structure")
assert(first_option_plan.source_prompt == "build a small shelter")
assert(first_option_plan.safety_budget.requires_preview == true)
assert(first_option_plan.safety_budget.requires_approval == true)
assert(first_option_plan.safety_budget.rollback_required == true)
assert(first_option_plan.safety_budget.ai_direct_world_mutation_allowed == false)
assert(first_option_plan.ai_disclosure.direct_world_mutation_by_ai == false)
assert(first_option_plan.provenance.generator == "openrealm_creator_kernel")
assert(first_option_plan.structures[1].placement_count
	== pending_options.candidate_options[1].planned_node_writes)
local options_chat_ok, options_chat_text =
	core.registered_chatcommands.nova.func("Planner", "options")
assert(options_chat_ok == true)
assert(options_chat_text:find("status=success action=build_options", 1, true))
assert(options_chat_text:find("selected_candidate=platform", 1, true))
assert(options_chat_text:find("option=platform", 1, true))
assert(options_chat_text:find("openrealm_plan=openrealm.plan.v1", 1, true))
local options_player_text = core.ai_agent_plugin.format_player_reply(pending_options)
...
assert(generated_tower_reply.agentic_tool_success_required == true)
...
assert(generated_tower_trace.response.agentic_tool_success_required == true)
...
local pending_generated_tower =
	core.ai_agent_plugin.handle_command("PlannerTower", "pending plan", {})
assert(pending_generated_tower.ok == true)
assert(pending_generated_tower.agentic_tool_success_required == true)
...
rawset(_G, "test_ai_agent_plugin_prompt_eval_surface", function()
	...
	async_eval_done[1]({
		...
		adapter_name = "mock-eval-stone-bridge",
		elapsed_us = 80000,
		response = {
			agentic_execution = true,
	...
	assert(eval_cases.stone_bridge ~= nil)
	...
	assert(eval_cases.fire_only_strict.prompt == "build me a fire and only a fire")
	assert(eval_cases.fire_only_strict.reply.status == "pending_approval")
	assert(eval_cases.fire_only_strict.reply.build_kind == "fire")
	assert(eval_cases.fire_only_strict.reply.build_material_name == "fire")
	assert(eval_cases.fire_only_strict.reply.planned_node_writes == 1)
	assert(eval_cases.fire_only_strict.trace.route == "deterministic_build_parser")
	assert(eval_cases.fire_only_strict.cleanup.action == "discard_approval")
	assert(eval_cases.fire_only_strict.cleanup.status == "success")
	...
	assert(eval_cases.stone_bridge.queued_status == "queued")
	assert(eval_cases.stone_bridge.initial_trace.route == "agentic_build_planner")
	assert(eval_cases.stone_bridge.final_status == "pending_approval")
	assert(eval_cases.stone_bridge.final_reply.selected_candidate_id
	...
	assert(eval_cases.player_agent_loop.queued_status == "queued")
	assert(eval_cases.player_agent_loop.final_status == "pending_approval")
	assert(eval_cases.player_agent_loop.final_reply.selected_candidate_id
		== "generated_openrealm_lakeside_village")
	assert(eval_cases.player_agent_loop.options_reply.status == "success")
	assert(eval_cases.player_agent_loop.options_reply.selected_candidate_id
		== "generated_openrealm_lakeside_village")
	assert(eval_cases.player_agent_loop.options_trace.route == "natural_chat_review")
	...
	assert(eval_cases.player_agent_loop.pending_plan_trace.response.status == "success")
	assert(eval_cases.player_agent_loop.discard_reply.action == "discard_approval")
	assert(eval_cases.player_agent_loop.discard_reply.status == "success")
	assert(eval_cases.player_agent_loop.discard_trace.route == "natural_chat_review")
	assert(eval_cases.player_agent_loop.discard_trace.action == "discard_approval")
	assert(eval_cases.player_agent_loop.discard_trace.public_prompt == "no")
	assert(eval_cases.player_agent_loop.discard_trace.response.status == "success")
	assert(eval_cases.player_agent_loop.after_discard_reply.status == "blocked")
	assert(eval_cases.player_agent_loop.after_discard_reply.reason == "no_pending_approval")
	assert(eval_cases.player_agent_loop.after_discard_trace.route == "natural_chat_review")
	assert(eval_cases.player_agent_loop.after_discard_trace.action == "pending_plan")
	assert(eval_cases.player_agent_loop.after_discard_trace.public_prompt == "pending plan")
	...
	local fire_only_command_ok, fire_only_command_message =
		core.registered_chatcommands.ai_agent_eval.func("EvalCommand", "case=fire_only")
	assert(fire_only_command_ok == true)
	local fire_only_command_report = core.parse_json(fire_only_command_message)
	assert(fire_only_command_report.operation == "ai_agent_plugin.run_prompt_eval")
	assert(fire_only_command_report.ok == true)
	assert(fire_only_command_report.status == "pass")
	assert(#fire_only_command_report.cases == 1)
	assert(fire_only_command_report.cases[1].case_id == "fire_only_strict")
