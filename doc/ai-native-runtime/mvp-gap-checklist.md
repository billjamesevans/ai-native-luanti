...
| Follow/come product behavior | `already_proven` | Follow queues bounded continuous helper-entity movement through `core.ai_entity_ops.move` and `core.queue_ai_task`; come remains a bounded one-shot helper move. | Keep expanding pathfinding beyond the current direct-line slice under task, wall-clock, and distance budgets. |
