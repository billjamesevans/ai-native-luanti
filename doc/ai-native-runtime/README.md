...
- Agent memory refresh: `python3 util/ai_native_agent_memory_refresh.py --agents-sdk-log local/logs/agents-sdk-model-adapter.jsonl --action-log local/logs/luanti-debug.log --candidate-queue-output local/benchmarks/ai-agent-eval-candidate-queue.json --case-pack-output local/benchmarks/ai-agent-prompt-eval-case-pack.json --generated-at 2026-06-30T00:00:00Z`
- Agentic build selection contract: Agents SDK build-planning responses may change the pending preview only through `response.selected_option_id` or `response.tool_decisions.build_option.selected_option_id` matching a bounded candidate from Luanti; healthy live runs should expose `tool_decision_source = agents_sdk_function_tool`, while fallback sources feed the eval/improvement loop.
