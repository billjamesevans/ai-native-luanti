...
def write_probe_world(world_dir: Path, generated_at: str, max_bytes: int) -> None:
    ...
    (mod_dir / "init.lua").write_text(
        "\n".join([
            ...
            "    core.load_area({ x = -2, y = 10, z = 118 }, { x = 12, y = 14, z = 122 })",
            "  end",
            ...
            "  core.set_node(cancel_plan_pos, { name = \"air\" })",
            "  core.set_node(no_pos, { name = hazard_node })",
            "end",
            "",
            ...
            "  local build_status_context = handle(\"BuilderLive\", \"status\", {})",
            ...
            "",
            "  local cancel_plan_pending = handle(\"DeclineLive\", \"build marker\", live_context(cancel_plan_pos))",
            "  local cancel_plan_reply = handle(\"DeclineLive\", \"cancel plan\", {})",
            "  local cancel_plan_after = handle(\"DeclineLive\", \"pending plan\", {})",
            "  local no_pending = handle(\"NoLive\", \"repair\", live_context(no_pos))",
            "  local no_reply = handle(\"NoLive\", \"no\", {})",
            "  local no_after = handle(\"NoLive\", \"pending plan\", {})",
            "",
            ...
            "  local commands_reply = handle(\"BuilderLive\", \"commands\", {})",
            "  local tasks_reply = handle(\"BuilderLive\", \"tasks\", {})",
            "  local clean_defend_gated = handle(\"BuilderLive\", \"defend\", {})",
            "  local clean_import_gated = handle(\"BuilderLive\", \"import plan\", {})",
            ...
            "  end",
            "  local status_surface_by_id = {}",
            "  for _, surface in ipairs(build_status_context.product_surfaces or {}) do",
            "    status_surface_by_id[surface.surface_id] = surface",
            "  end",
            ...
            "  end",
            "  local function status_surface_ready(surface_id)",
            "    local surface = status_surface_by_id[surface_id] or {}",
            "    return surface.required_capabilities_granted == true",
            "  end",
            "",
            ...
            "      },",
            "      pending_declines = {",
            "        cancel_plan_pending_status = cancel_plan_pending.status,",
            "        cancel_plan_status = cancel_plan_reply.status,",
            "        cancel_plan_action = cancel_plan_reply.action,",
            "        cancel_plan_approval_id = cancel_plan_reply.approval_id,",
            "        cancel_plan_pending_after_status = cancel_plan_after.status,",
            "        no_pending_status = no_pending.status,",
            "        no_status = no_reply.status,",
            "        no_action = no_reply.action,",
            "        no_approval_id = no_reply.approval_id,",
            "        no_pending_after_status = no_after.status,",
            "      },",
            "      task_control = {",
            ...
            "      },",
            "      surfaces = {",
            "        status_context_checked = build_status_context.status == \"success\"",
            "          and build_status_context.known_task_count == 0",
            "          and build_status_context.pending_approval",
            "          and build_status_context.pending_approval.approval_id == build_pending.approval_id",
            "          and build_status_context.pending_approval.pending_action == \"build\"",
            "          and status_surface_ready(\"builder\")",
            "          and status_surface_ready(\"repair\"),",
            "        guide_command_checked = guide_reply.status == \"success\" and guide_reply.surfaces.builder == true,",
            "        commands_alias_checked = commands_reply.status == \"success\"",
            "          and commands_reply.action == \"guide\"",
            "          and commands_reply.surfaces.builder == true",
            "          and #(commands_reply.product_surfaces or {}) == 5,",
            ...
            "        defender_clean_command_gated = clean_defend_gated.status == \"blocked\"",
            "          and clean_defend_gated.reason == \"surface_capability_not_granted\"",
            "          and clean_defend_gated.task_id == nil",
            "          and clean_defend_gated.required_capabilities_granted == false,",
            "        importer_clean_command_gated = clean_import_gated.status == \"blocked\"",
            "          and clean_import_gated.reason == \"surface_capability_not_granted\"",
            "          and clean_import_gated.task_id == nil",
            "          and clean_import_gated.required_capabilities_granted == false,",
            ...
            "        pending_decline_alias_checked = cancel_plan_pending.status == \"pending_approval\"",
            "          and cancel_plan_reply.status == \"success\"",
            "          and cancel_plan_reply.action == \"discard_approval\"",
            "          and cancel_plan_reply.approval_id == cancel_plan_pending.approval_id",
            "          and cancel_plan_after.status == \"blocked\"",
            "          and no_pending.status == \"pending_approval\"",
            "          and no_reply.status == \"success\"",
            "          and no_reply.action == \"discard_approval\"",
            "          and no_reply.approval_id == no_pending.approval_id",
            "          and no_after.status == \"blocked\",",
            ...
            "      clean_gated_command_count = (clean_defend_gated.status == \"blocked\" and 1 or 0)",
            "        + (clean_import_gated.status == \"blocked\" and 1 or 0),",
            "      status_context_count = build_status_context.status == \"success\" and 1 or 0,",
            "      commands_alias_count = commands_reply.status == \"success\" and 1 or 0,",
            "      pending_decline_alias_count = (cancel_plan_reply.status == \"success\" and 1 or 0)",
            "        + (no_reply.status == \"success\" and 1 or 0),",
...
def validate_live_result(payload: dict, max_bytes: int = DEFAULT_MAX_BYTES) -> dict:
    ...
    for field in (
        ...
        "commands_alias_checked",
        "status_context_checked",
    ...
    return {
        ...
        "agent_product_loop_clean_gated_commands": summary["clean_gated_command_count"],
        "agent_product_loop_status_contexts": summary["status_context_count"],
        "agent_product_loop_commands_aliases": summary["commands_alias_count"],
        "agent_product_loop_pending_decline_aliases": summary["pending_decline_alias_count"],
        ...
        "agent_product_loop_commands_alias_checked": True,
        "agent_product_loop_pending_decline_alias_checked": True,
        ...
        "agent_product_loop_defender_clean_command_gated": True,
        "agent_product_loop_importer_clean_command_gated": True,
