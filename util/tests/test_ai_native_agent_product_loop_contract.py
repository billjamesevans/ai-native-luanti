...
class AgentProductLoopContractTests(unittest.TestCase):
    def test_first_party_agent_plugin_documents_product_loop_commands(self):
        ...
        for phrase in (
            ...
            "`core.ai_agent_plugin.ensure_product_agents",
            "`core.ai_agent_plugin.get_product_surfaces",
            "`nova_agent:<player>:builder`",
            "`nova_agent:<player>:repair`",
            "`nova_agent:<player>:guide`",
            "`nova_agent:<player>:defender`",
            "`nova_agent:<player>:importer`",
            "default clean-profile grant boundary",
            "Defender Agent",
            "Importer Agent",
            "`guide`",
            "`importer`",
            ...
            "Targeted task, audit, rollback-review, and approval commands",
            "Pending-plan edit is also read-only/player-local",
    ...
    def test_first_party_agent_plugin_uses_runtime_surfaces_not_raw_world_writes(self):
        ...
        for phrase in (
            ...
            "core.ai_entity_ops.move",
            "PRODUCT_SURFACES",
            "plugin.ensure_product_agents",
            "plugin.ensure_surface_agent",
            "plugin.get_product_surfaces",
            "plugin.format_reply",
            "format_command_reply",
            "surface_agent_id_for",
            "default_clean_profile_grant",
